// $Header: FuzzyClusterTool.cxx,v 1.0 2003/03/02 04:11:20 musat Exp $
#include "ICluster.h"
...
if(dataSetSize >= maxClusters) {
    ...
    double ene[nClusters];                 // total energy in calorimeter
    Vector pCluster[nClusters];            // cluster position
    // energy per layer
    std::vector<double> eneLayer[nClusters];
    // Vector of average position per layer
    std::vector<Vector> pLayer[nClusters];
    
    // Vector of quadratic spread per layer
    std::vector<Vector> rmsLayer[nClusters];
